...
const APP = {
  ...
  checkoutClickHandler(event) {
    ...
    link.target = "_blank";
    link.onclick = "window.open('checkout.html');";
    link.click();
    */
  },
  ...
  cacheData() {
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      formData.append(
        "toBuyQuantity",
        quantities[i].options[quantities[i].selectedIndex].text
      );
  ...
  buildRows() {
    ...
    for (const [key, value] of formData) {
      if (key === "toBuyPicture") {
        tr[i].innerHTML +=
          '<td style="border-bottom: 1px solid; vertical-align: middle;" colNum="' +
          col +
          '" class="' +
          key +
          '"><img src="' +
